...
class Spotibrarian:
    def __init__(self):
        ...
        self._sp = spotipy.Spotify(auth_manager=SpotifyOAuth(scope=scope, client_id = cid, client_secret = cs, redirect_uri="http://localhost:8888/callback"))
        self._init_library()
            ...
            try:
                if self._current_song is not None:
                    if self._current_song in self._library:
                        self._library[self._current_song]['tags'] = sorted(list(set([tag.strip() for tag in str(self._ui.ptTags.toPlainText()).replace('\n','').replace('\t','').split(',') if tag.strip() != ''])))
                        self._update_library_view()
                        self._update_tags()
                if self._ui.twSongs.currentItem() is not None:
                    ...
                    self._ui.ptTags.clear()
                    image = qtg.QImage()
                    image.loadFromData(urllib.request.urlopen("https://www.solidbackgrounds.com/images/950x350/950x350-arsenic-solid-color-background.jpg").read())
                    self._pixmap = qtg.QPixmap(image).scaled(128, 128, qtc.Qt.KeepAspectRatio)
                    self._ui.albumArt.setPixmap(self._pixmap)
